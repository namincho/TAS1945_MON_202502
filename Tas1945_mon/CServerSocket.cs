...
namespace CSockets
{
    public class CServerSocket
    {
        ...
        public event ConnectionDelegate OnReadBytes;
        public event ConnectionDelegate OnWrite;
        public event ConnectionDelegate OnWriteBytes;
        ...
        public CServerSocket(string IP, int port)
        {
            try
            {
                mPort = port;

                serverEndPoint = new IPEndPoint(IPAddress.Parse(IP),mPort); //Any, mPort);
            }
            catch (Exception ex)
            {
                if (OnError != null)
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Buffer.BlockCopy(socketData.dataBuffer, 0, mBytesReceived, 0, iRx);
                    if (OnReadBytes != null)
                        OnReadBytes(socketData.m_currentSocket);
        ...
        public bool SendBytes(byte[] byData, int SocketIndex, int Length)
        {
            if ((Clients.Count - 1) >= SocketIndex)
            {
                Socket workerSocket = (Socket)Clients[SocketIndex];
                try
                {
                    int NumBytes = workerSocket.Send(byData, Length, SocketFlags.None);
                    if (NumBytes == Length)
                    {
                        if (OnWriteBytes != null)
                        {
