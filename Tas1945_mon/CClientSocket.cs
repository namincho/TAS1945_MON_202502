...
namespace CSockets
{
    public class CClientSocket
    {
        ...
        /// <summary>
        /// Message length received by the Socket
        /// </summary>
        public int ReceivedLength
        {
            get
            {
                int itemp = iBytesReceived;
                iBytesReceived = 0;
                return (itemp);
            }
        }
        ...
        public CClientSocket(string IP, int port)
        {
            try
            {
                ...
                //IPHostEntry ipss = Dns.EndGetHostEntry("google.com");//Dns.GetHostEntry(mRemoteAddress);
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            try
            {
                ...
                else
                {
                    ...
                    Buffer.BlockCopy(dataBuffer, 0, mBytesReceived, 0, iRx);
        ...
        public bool SendBytes(byte[] byData, int Length)
        {
            try
            {
                int NumBytes = mainSocket.Send(byData, 0, Length, SocketFlags.None);
                if (NumBytes == Length)
                {
                    if (OnWrite != null)
                    {
